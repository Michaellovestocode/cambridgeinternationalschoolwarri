...
public function manualResultFilling()
{
    $subjects = $this->availableExamSubjects();
    $classes = $this->availableExamClasses();
    $classesBySubject = $this->classesBySubject($subjects, $classes);
    ...
    return view('admin.exams.manual-result-filling', compact(
        'subjects',
        'classesBySubject',
        'activeSession',
        'activeTerm'
...
public function startManualResultFilling(Request $request)
{
    $validated = $request->validate([
        'subject_id' => 'required|exists:subjects,id',
    ...
    $assignmentErrors = $this->validateExamAssignment((int) $validated['subject_id'], [(int) $validated['class_id']]);
    ...
    if (! $activeSession || ! $activeTerm) {
        return back()->with('error', 'No active session or term found. Please set the active academic period first.')->withInput();
    }

    $subject = Subject::findOrFail($validated['subject_id']);
    $class = SchoolClass::findOrFail($validated['class_id']);
    $title = "Manual Results - {$subject->name} - {$activeSession->name} - {$activeTerm->name}";
    ...
    $exam = Exam::firstOrCreate(
        ...
        [
            'description' => "Manual result entry for {$subject->name}.",
            'subject' => $subject->name,
            'duration_minutes' => 1,
            'total_marks' => 100,
            'pass_mark' => 50,
            'instructions' => 'Manual result filling shortcut.',
    ...
    $exam->classes()->syncWithoutDetaching([$class->id]);

    return redirect()->route('admin.exam.manual-scores', [
        'exam' => $exam->id,
        'class_id' => $class->id,
    ]);
}
