        ...
        <form action="{{ route('admin.subjects.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="class_level"
                    ...
                    <option value="all" {{ old('class_level') === 'all' ? 'selected' : '' }}>All Classes</option>
                    <option value="creche" {{ old('class_level') === 'creche' ? 'selected' : '' }}>Early Years</option>
                    <option value="primary" {{ old('class_level') === 'primary' ? 'selected' : '' }}>Primary</option>
                    <option value="junior" {{ old('class_level') === 'junior' ? 'selected' : '' }}>Junior Secondary</option>
                    <option value="senior" {{ old('class_level') === 'senior' ? 'selected' : '' }}>Senior Secondary</option>
                </select>
