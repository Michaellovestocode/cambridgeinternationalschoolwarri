...
<div class="mx-auto max-w-4xl space-y-5">
    <div class="flex flex-col gap-4 rounded-2xl bg-white p-5 shadow sm:flex-row sm:items-center sm:justify-between"><div><p class="text-xs font-bold uppercase tracking-wide text-blue-700">Clinic Visit Record</p><h1 class="mt-1 text-2xl font-black text-gray-900">{{ $visit->display_name }}</h1><p class="text-sm text-gray-500">{{ $visit->visited_at->format('j M Y, g:i A') }} · {{ $visit->person?->class?->display_name ?: ucfirst($visit->patient_type) }}</p></div>@if($visit->student)<a href="{{ route('clinic.students.show', $visit->student) }}" class="rounded-xl bg-gray-100 px-4 py-3 text-center text-sm font-bold text-gray-700">Back to History</a>@else<a href="{{ route('clinic.dashboard') }}" class="rounded-xl bg-gray-100 px-4 py-3 text-center text-sm font-bold text-gray-700">Back to Dashboard</a>@endif</div>
