<?php $__env->startSection('title', 'Parent Messages'); ?>
...
<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <a href="<?php echo e(route('parent.dashboard')); ?>" class="text-sm text-blue-600 hover:underline">Back to parent portal</a>
    </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg p-6 space-y-4">
            ...
            <?php if($children->isNotEmpty()): ?>
                <div class="rounded-2xl bg-blue-50 px-4 py-3 text-sm text-blue-700">
                    Linked children: <?php echo e($children->map(fn ($child) => $child->name . ' (' . ($child->class?->display_name ?? 'No class') . ')')->join(', ')); ?>

                </div>
            <?php endif; ?>

            <form method="POST" action="<?php echo e(route('parent.messages.store')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>
                <div>
                    <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Subject</label>
                    <input name="subject" value="<?php echo e(old('subject')); ?>" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500" placeholder="Optional short subject" />
                </div>
                <div>
                    <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Message</label>
                    <textarea name="body" rows="7" class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500" placeholder="Type your message to the school admin team..."><?php echo e(old('body')); ?></textarea>
                </div>
                    ...
                    <div class="border border-gray-100 rounded-2xl p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p class="text-xs text-gray-500">From <?php echo e($message->sender?->name ?? 'School Admin'); ?> • <?php echo e(ucfirst($message->sender?->role ?? 'staff')); ?></p>
                            </div>
    ...
    <div class="bg-white rounded-3xl shadow-lg p-6 space-y-4">
        <div class="flex items-center justify-between">
            ...
            <span class="text-xs font-semibold px-3 py-1 rounded-full bg-emerald-50 text-emerald-700"><?php echo e($sentMessages->count()); ?> sent</span>
        </div>
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="text-xs text-gray-500">
                                Delivered to <?php echo e($message->recipient_count === 1 ? $message->recipients->first() : $message->recipient_count . ' staff members'); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\modern-cbt-platform-for-highschool\resources\views\parent\messages\index.blade.php ENDPATH**/ ?>